...
def _overpass_post(query: str, timeout: int = 45):
    ...
    Mirror blacklist policy:
      - 429 / connection errors → permanently dead for this process run
        (rate-limited or unreachable — no point retrying)
      - 502 / 503 / 504          → skip THIS call but keep in rotation
        (transient server overload — may recover for the next query)
    """
    last_err = None
    transient_skip: set = set()   # skipped for this call only, NOT added to _dead_mirrors

    for mirror in OVERPASS_MIRRORS:
        if mirror in _dead_mirrors or mirror in transient_skip:
            continue
        try:
            r = requests.post(mirror, data={"data": query}, timeout=timeout)

            if r.status_code == 429:
                _warn(f"429 on {mirror} — rate limited, marking permanently dead")
                _dead_mirrors.add(mirror)
                continue

            if r.status_code in (502, 503, 504):
                _warn(f"HTTP {r.status_code} on {mirror} — transient error, trying next mirror")
                transient_skip.add(mirror)   # skip only for this call
                continue
            ...
            last_err = e
            _dead_mirrors.add(mirror)
            _warn(f"{mirror} failed: {type(e).__name__} — marking permanently dead")

    _warn("All Overpass mirrors exhausted.")
    return None
...
def build_and_save_graph(
    ...
    # ── Early exit: return existing graph if already built ─────────────────
    try:
        S = get_session_maker()
        with S() as s:
            ...
            if region:
                ...
                if graph:
                    ...
                    _ok(f"Graph for '{destination}' already exists in DB "
                        f"(v{graph.version}, {len(places)} places, {len(edges)} edges) — skipping build")
                    pd_ = {p.id: {
                        "id": p.id, "name": p.name,
        ...
        _warn(f"Pre-build DB check failed: {e} — proceeding with full build")
